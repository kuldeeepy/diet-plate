// Local dev server: serves pile/ and runs the /api/match function Vercel hosts in production.
// Key comes from AI_GATEWAY_API_KEY, or a .env.local in the repo root. Never commit that file.
...
async function api(req, res) {
  ...
  try {
    ...
    const hits = Object.values(out.scores).filter(s => s >= 0.5).length;
