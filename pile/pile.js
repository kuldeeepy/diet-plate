...
async function resolve(q) {
  ...
  const key = norm(q);
  if (cache.has(key)) return { ...rank(cache.get(key)), via: 'cache' };
  ...
  try {
    ...
    cache.set(key, j.scores);
    return { ...rank(j.scores), via: 'jev' };
  } catch (e) {
    clearTimeout(bail); loading = false; document.body.classList.remove('busy');
...
function rank(scores) {
  const r = items.filter(it => (scores[it.rec.id] ?? 0) >= 0.5)
                 .sort((a, b) => scores[b.rec.id] - scores[a.rec.id]);
  r.forEach(it => it.score = scores[it.rec.id]);
  return { hits: r.slice(0, 10), total: r.length };
}
